...
public class InitReplicationScenarioWithInternalNetwork extends InitReplicationScenario {
	...
	protected Datacenter createSuperDatacenter(CloudSim simulation) {
        ...
        for(int j = 0; j < SimulationConstParameters.HOST_SUPER; j++) {
        	...
        	((RackHost)host).setRackId(((RackHost)host).getHostCount()%5);
            hostList.add(host);
            if(currentRack == 0){ 
            	storageList.add(((RackSanStorage)createStorage(1000000000, 10.0, 5)).setRackId(currentRack));
            }
            if(((RackHost)host).getHostCount()%5 != currentRack){ 
            	currentRack++;
            	storageList.add(((RackSanStorage)createStorage(1000000000, 10.0, 5)).setRackId(currentRack));
            }
        }
